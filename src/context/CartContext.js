...
export const CartContextProvider = ({ children }) => {
    ...
    const [count, setCount] = useState(cart ? cart.reduce((total, product) => total + product.qty, 0) : 0);

    const [total, setTotal] = useState(cart ? cart.reduce((total, product) => total + (product.qty*product.price), 0) : 0);
    ...
    const addItem = (item) => {
        ...
        setCount(cart ? cart.reduce((total, product) => total + product.qty, 0) : 0);
        setTotal(cart ? cart.reduce((total, product) => total + (product.qty*product.price), 0) : 0);
    }
    ...
    const context = { cart, count, addItem, removeItem, total, deleteLine, deleteCart }
